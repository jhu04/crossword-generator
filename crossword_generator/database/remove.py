...
def remove(publish_type):
    ...
    Removes all documents of type `publish_type` from PUZZLE_COLLECTION. 
    """
    ...
    if publish_type.lower() == 'fake':
        if query_yes_no(question, default='no'):
            action = const.PUZZLE_COLLECTION.delete_many({'puzzle_meta.publishType': PublishType.FAKE.value})
